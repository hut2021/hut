...
interface IERC721 {
    ...
    function tokenCombatPower(uint256 tokenId) external view returns (uint256);
}
...
contract USDTWrapper {
    ...
    function stake(address from, uint256 amount) internal virtual {
...
contract lolNFTStakingPool is USDTWrapper, ERC721TokenReceiver {
    ...
    //
    IERC721 nftToken;
    mapping(address => OrderIdMap.Map) orderMap;
    uint256 public minPower;
    address operator = 0xf9B21Dc777F72cBeEF1a0FB49b0eD9B6F4bbD9FB;
    ...
    constructor(
        ...
        rewardToken = IERC20(outToken_);
        nftToken = IERC721(inNFTToken_);
        starttime = starttime_;

        rewardRate = dayOutAmount_.div(oneDay_);
    ...
    function stake(address from, uint256 tokenId)
    internal
    override
    updateReward(from)
    checkStart
    {
        uint256 amount = nftToken.tokenCombatPower(tokenId);
        require(amount >= minPower, 'Combat power is too low');
        require(orderMap[from].set(tokenId), "map fail");
        uint256 newDeposit = deposits[from].add(amount);
        deposits[from] = newDeposit;
        super.stake(from, amount);
    ...
    checkStart
    {
        require(orderMap[msg.sender].contains(tokenId), "not exist");
        uint256 amount = nftToken.tokenCombatPower(tokenId);
        require(amount > 0, 'Cannot withdraw 0');
        deposits[msg.sender] = deposits[msg.sender].sub(amount);
        super.withdraw(amount);
        emit Withdrawn(msg.sender, amount);
        orderMap[msg.sender].remove(tokenId);
        nftToken.safeTransferFrom(address(this), msg.sender, tokenId);
    }

    function exit() external {
        uint256 orderLen = orderMap[msg.sender].lengthMemory();
        require(orderLen > 0, "len0");
        uint256[] memory temp = orderMap[msg.sender].getArrMemory();
    ...
    function setPoolTime(uint256 _start, uint256 _end) public checkOperator {
        starttime = _start;
        periodFinish = _end;
    }

    function getList(address _addr) public view returns (uint256[] memory ids, uint256[] memory powers){
        uint256 orderLen = orderMap[_addr].lengthMemory();
        if (orderLen > 0) {
            ids = new uint256[](orderLen);
            powers = new uint256[](orderLen);
            uint256[] memory temp = orderMap[_addr].getArrMemory();
            for (uint256 i = 0; i < orderLen; i++) {
                ids[i] = temp[i];
                powers[i] = nftToken.tokenCombatPower(temp[i]);
            }
        }
    }
